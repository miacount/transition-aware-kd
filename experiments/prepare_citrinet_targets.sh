...
LBS_TEACHER=stt_en_citrinet_512
CHIME_TEACHER=nemo_experiments/teacher-citrinet512-chime3-adapted-s1/best-teacher-citrinet512-chime3-adapted-s1.nemo
LBS_ARTIFACT=$(find /root/.cache/torch/NeMo -type f -path '*/stt_en_citrinet_512/*.nemo' -print -quit)
...
if [[ "$SCOPE" == lbs || "$SCOPE" == all ]]; then
  python scripts/evaluate_teacher.py --teacher "$LBS_TEACHER"     --manifest dev_clean=data/dev_clean.json --manifest dev_other=data/dev_other.json     --manifest test_clean=data/test_clean.json --manifest test_other=data/test_other.json     | tee analysis/citrinet512_teacher_lbs.txt
  build_all_targets data/train_clean_100.json "$LBS_TEACHER" "$LBS_ARTIFACT" train_clean_100.citrinet512
fi

if [[ "$SCOPE" == chime3 || "$SCOPE" == all ]]; then
  if [[ ! -f "$CHIME_TEACHER" ]]; then
    python scripts/finetune_teacher_ted3.py --config-name teacher_finetune_citrinet512_chime3
  fi
  python scripts/evaluate_teacher.py --teacher "$CHIME_TEACHER"     --manifest dev_real=data/chime3_dev_real_enhanced.json     --manifest dev_simu=data/chime3_dev_simu_enhanced.json     --manifest eval_real=data/chime3_eval_real_enhanced.json     --manifest eval_simu=data/chime3_eval_simu_enhanced.json     | tee analysis/citrinet512_teacher_chime3.txt
  build_all_targets data/chime3_train_enhanced.json "$CHIME_TEACHER" "$CHIME_TEACHER" chime3_train.citrinet512_adapted
fi
